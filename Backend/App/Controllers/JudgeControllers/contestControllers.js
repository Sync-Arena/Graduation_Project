...
export const createContest = asyncHandler(async (req, res, next) => {
	...
	if (!contestName) throw new AppError("Please add name for the contest", 400)

	if (!startTime)
		throw new AppError("Please add start time for the contest", 400)

	problems = JSON.parse(problems)
	if (!problems)
		throw new AppError("The contest must have at least one problem", 400)

	if (isNaN(durationInMinutes))
		throw new AppError("The duration must be a positive number", 400)
