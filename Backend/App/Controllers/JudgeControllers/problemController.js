...
export const createProblem = asyncHandler(async (req, res, next) => {
	...
	try {
		problemInfo = await axios.get(
			getURL("problem.info", { problemId: polygonId })
		)
	} catch (err) {
	...
	const problemStatements = await axios.get(
		getURL("problem.statements", { problemId: polygonId })
	)
	const { english: statement } = problemStatements.data.result

	const problemTests = await axios.get(
		getURL("problem.tests", { problemId: polygonId, testset: "tests" })
	)
	const testsIndexs = []
	...
	const problemCheckerName = await axios.get(
		getURL("problem.checker", { problemId: polygonId })
	)
	...
	try {
		let problemData
		if (!problem)
			problemData = await problemDataModel.create(problemDataCreateObj)
		else
			problemData = await problemDataModel.findByIdAndUpdate(
				problem.ProblemDataId,
				problemDataCreateObj,
		...
		for (let i = 0; i < testsIndexs.length; i++) {
			...
			if (!problem || problem.testCases.length <= i)
				...
				testCase = await problemTestCasesModel.findByIdAndUpdate(
					problem.testCases[i],
					testCaseData,
					{ new: true, validate: true }
				)
		...
		if (!problem) newProblem = await problemModel.create(newproblemData)
			...
			newProblem = await problemModel.findByIdAndUpdate(
				problem._id,
				newproblemData,
				{ new: true, validate: true }
			)
		...
		return res
			.status(201)
			.json({ status: "problem created", problem: newProblem })
	} catch (err) {
		next(
			new AppError(
