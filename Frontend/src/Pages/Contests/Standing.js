...
function Standing() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const standing = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/judge/contest/${contestId.id}/standing`, config)
    ...
    let standingBody = standingArray.map((row, index) => {
        return (
            <tr key={index} className={`${index % 2 === 0 ? "bg-second_bg_color_dark" : ""}`}>
                ...
                <td className="px-3 py-4 text-center">{row.submissionObject.solvedProblems}</td>
    ...
    return (
        <div className="overflow-x-auto mt-10 flex justify-center">
            {
                loading ? <div className="text-white text-3xl py-8">Loading...</div>
                    :
