...
function Standing() {
    ...
    let cols = [<col style={{ width: "3%" }} />, <col style={{ width: "10%" }} />, <col style={{ width: "3%" }} />, <col style={{ width: "4%" }} />]
    let ths = []
    for (let i = 0, ch = 'A'; i < noOfPoroblems; ++i, ch++) {
        cols.push(<col style={{ width: `${widthForOneCol}%` }} />)
        ths.push(<th scope="col" className="px-3 py-3 text-center">
            {String.fromCharCode("A".charCodeAt(0) + i)}
        </th>)
    }
    let test = [[1, "Ahmed", 2, 900, { solved: true, tries: 2 }, { solved: true, tries: 1 }, { sovled: false, tries: 2 }, { solved: false, tries: 0 }],
    [1, "Ahmed", 2, 900, { solved: true, tries: 2 }, { solved: true, tries: 1 }, { sovled: false, tries: 2 }, { solved: false, tries: 0 }]
    ]
    let kk = test.map((el, index) =>
        <tr key={index} className={`${index % 2 === 0 ? "bg-second_bg_color_dark" : ""}`}>
            {el.map((ell, index) => {
                if (ell.constructor.name == "Object") {
                    if (ell.solved) {
                        if (ell.tries == 1)
                            return (<td className="px-3 py-4 text-green-600 text-center font-bold">
                                +
                            </td>)
                        else
                            return (<td className="px-3 py-4 text-green-600 text-center font-bold">
                    ...
                    return (<td className="px-3 py-4">{ell}</td>)
                }
            }
            )}
        </tr>
